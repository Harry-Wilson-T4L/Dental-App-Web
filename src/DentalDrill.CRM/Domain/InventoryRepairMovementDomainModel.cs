...
{
    public class InventoryRepairMovementDomainModel : InventoryMovementDomainModelBase, IInventoryRepairMovement
    {
        private readonly IHandpieceRequiredPartManager handpieceRequiredPartManager;

        public InventoryRepairMovementDomainModel(
        ...
        public async Task AllocateAsync()
        {
            var updateableEntity = await this.Repository.Query<InventoryMovement>().SingleAsync(x => x.Id == this.Id);
            var change = await this.PrepareTrackChangeAsync(ChangeAction.Modify, updateableEntity);
            updateableEntity.Status = InventoryMovementStatus.Allocated;
            await this.Repository.UpdateAsync(updateableEntity);
            await this.TrackChangeAsync(change, updateableEntity);
        ...
        public async Task DeallocateAsync()
        {
            var updateableEntity = await this.Repository.Query<InventoryMovement>().SingleAsync(x => x.Id == this.Id);
            var change = await this.PrepareTrackChangeAsync(ChangeAction.Modify, updateableEntity);
            updateableEntity.Status = InventoryMovementStatus.Waiting;
            await this.Repository.UpdateAsync(updateableEntity);
            await this.TrackChangeAsync(change, updateableEntity);
        ...
        public async Task SetPriceInternalAsync(Decimal price)
        {
            var updateableEntity = await this.Repository.Query<InventoryMovement>().SingleAsync(x => x.Id == this.Id);
            var change = await this.PrepareTrackChangeAsync(ChangeAction.Modify, updateableEntity);
            updateableEntity.Price = price;
            await this.Repository.UpdateAsync(updateableEntity);
            await this.TrackChangeAsync(change, updateableEntity);
        ...
        public async Task ClearPriceInternalAsync()
        {
            var updateableEntity = await this.Repository.Query<InventoryMovement>().SingleAsync(x => x.Id == this.Id);
            var change = await this.PrepareTrackChangeAsync(ChangeAction.Modify, updateableEntity);
            updateableEntity.Price = null;
            await this.Repository.UpdateAsync(updateableEntity);
            await this.TrackChangeAsync(change, updateableEntity);
