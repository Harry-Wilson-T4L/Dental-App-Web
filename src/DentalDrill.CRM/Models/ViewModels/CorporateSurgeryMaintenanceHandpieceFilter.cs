...
{
    public class CorporateSurgeryMaintenanceHandpieceFilter
    {
        ...
        public IQueryable<Handpiece> ApplyToQuery(IQueryable<Handpiece> query)
        {
            ...
            {
                query = query.Where(x => x.Job.ClientId == this.ClientId);
            }
        ...
        public (String QueryText, Object[] QueryParameters) GenerateWhereCondition(String jobAlias, String handpieceAlias, Boolean prefixWithAnd)
        {
            ...
            if (this.ClientId.HasValue)
            {
                ...
                queryText.Append($"{jobAlias}.[ClientId] = @filterClientId");
                queryParameters.Add(new SqlParameter("filterClientId", SqlDbType.UniqueIdentifier) { Value = this.ClientId.Value });
            }
