...
{
    public class SurgeryStatusesReportExcelExporter : ExcelExporterBase
    {
        private readonly IClient client;
        private readonly IClientStatusesReports report;
        private readonly DateTime from;
        private readonly DateTime to;

        public SurgeryStatusesReportExcelExporter(IClient client, IClientStatusesReports report, DateTime from, DateTime to)
        {
            this.client = client;
            this.report = report;
            this.from = from;
        ...
        protected override async Task ProcessWorkbook()
        {
            ...
            {
                await this.ProcessBrandsSheet();
            }
        }

        protected async Task ProcessBrandsSheet()
        {
            this.BrandsSheet.ConfigureColumn(widthInPixels: 200); // Brand
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Received
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Being Est.
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Est. Comp.
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Est. Sent
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Being Rep.
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Rdy. For Ret.
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Unrepaired
            this.BrandsSheet.ConfigureColumn(widthInPixels: 100); // Complete

            var queryResult = await this.report.GetBrandsForEntirePeriodAsync();
            await this.RenderInsights(queryResult);
            await this.RenderHeader("Brand");
            foreach (var brand in queryResult.OrderBy(x => x.Brand))
            {
                using (var row = this.BrandsSheet.CreateRowExporter())
                {
                    ...
                    row.CreateCell(brand.StatusReceived, this.ContentStyle);
                    row.CreateCell(brand.StatusBeingEstimated, this.ContentStyle);
                    row.CreateCell(brand.StatusWaitingForApproval, this.ContentStyle);
                    row.CreateCell(brand.StatusEstimateSent, this.ContentStyle);
                    row.CreateCell(brand.StatusBeingRepaired, this.ContentStyle);
                    row.CreateCell(brand.StatusReadyToReturn, this.ContentStyle);
                    row.CreateCell(brand.StatusCancelled, this.ContentStyle);
                    row.CreateCell(brand.StatusSentComplete, this.ContentStyle);
                }
            }
        }

        private async Task RenderInsights(List<SurgeryReportStatusItem> data)
        {
        ...
        {
            using var header = this.BrandsSheet.CreateRowExporter();
