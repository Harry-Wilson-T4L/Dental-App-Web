...
{
    public class SurgeryBrandsReportExcelExporter : ExcelExporterBase
    {
        private readonly IClient client;
        private readonly IClientBrandsReports report;
        ...
        public SurgeryBrandsReportExcelExporter(IClient client, IClientBrandsReports report, DateTime @from, DateTime to, ReportDateAggregate dateAggregate, HashSet<String> reportFields)
        {
            this.client = client;
            this.report = report;
            this.from = from;
        ...
        protected IWorksheetExporter BrandsModelsSheet { get; private set; }
        ...
        protected override async Task ProcessWorkbook()
        {
            ...
            using (this.BrandsModelsSheet = this.Workbook.CreateWorksheetExporter("Brands and models"))
            {
        ...
        {
            this.BrandsModelsSheet.ConfigureColumn(widthInPixels: 200);
            var fieldsCount = this.ComputeFieldsCount() * this.dateRanges.Count;
            for (var i = 0; i < fieldsCount; i++)
            {
                this.BrandsModelsSheet.ConfigureColumn(widthInPixels: 89);
            }

            await this.RenderHeader(this.BrandsModelsSheet, "Brand / Model");
            switch (this.dateAggregate)
            {
                case ReportDateAggregate.EntirePeriod:
                {
                    ...
                    foreach (var brand in queryResult.OrderBy(x => x.Brand))
                    {
                        using (var row = this.BrandsModelsSheet.CreateRowExporter())
                        {
                        ...
                        var models = modelsResult.Where(x => x.Brand == brand.Brand).OrderBy(x => x.Model).ToList();
                        foreach (var model in models)
                        {
                            using (var row = this.BrandsModelsSheet.CreateRowExporter())
                            {
                                row.CreateCell(model.Model, this.ContentStyle);
            ...
            await this.MergeHeader(this.BrandsModelsSheet);
        }

        private async Task RenderInsights(IWorksheetExporter sheet)
        {
            var data = await this.report.GetBrandsForEntirePeriodAsync();
