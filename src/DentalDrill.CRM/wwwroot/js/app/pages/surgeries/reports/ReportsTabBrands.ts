namespace DentalDrill.CRM.Pages.Surgeries.Reports {
    ...
    export class ReportsTabBrands extends ReportsTabBase {
        ...
        private async exportPdf(): Promise<void> {
            ...
            const table = new ReportTable<ReportBrandModelItem>();
            table.rowHeight = 23;
            ...
            if (detailRows.length > 0) {
                // Models
                const dataSet: ReportBrandModelItem[] = this._dataSource.data<ReportBrandItem>().map(x => x as ReportBrandModelItem);
                const models: ReportBrandModelItem[] = this._dataSourceModels.data<ReportBrandModelItem>().map(x => x as ReportBrandModelItem);
        ...
        async applyGlobalFilters(from: Date, to: Date): Promise<void> {
        ...
        private async toggleInsights(): Promise<void> {
            ...
            const data = this._dataSource.data<ReportBrandItem>();
