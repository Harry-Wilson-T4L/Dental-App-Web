namespace DentalDrill.CRM.Pages.GlobalReports.Reports {
    ...
    export class ReportsTabGeneralStats extends ReportsPageTabGridWithInsightsBase<GlobalReportsPageIdentifier, ReportsPageDateRangeGlobalFilters> {
        async initializeInsights(): Promise<void> {
            ...
            await this.insightsContainer.addChartsAsync(async charts => {
                const statusDataSource = await this.createDataSource({ transport: { read: { data: { DateAggregate: "EntirePeriod", Fields: ["HandpieceStatus"] } } } });
                const brandsDataSource = await this.createDataSource({ transport: { read: { data: { DateAggregate: "EntirePeriod", Fields: ["Brand"] } } } });
                {
                    const collection = new Collection<ReportItem>(statusDataSource.data<ReportItem>().map(x => x));
                    charts.createPieChart()
                        ...
                        .initialize(
                            ...
                            }).select(x => ({ category: x.key, value: x.items.count() })).toArray(),
                            "{0:n0}",
                            `#: category #: \n #: value #`,
                            undefined);
                }
                {
                    const collection = new Collection<ReportItem>(brandsDataSource.data<ReportItem>().map(x => x));
                    charts.createPieChart()
                        .override(options => {
                            options.seriesDefaults.labels.visible = false;
                            options.tooltip.template = `#: category #: #: value #`;
                        ...
                        .initialize(
                            "Brands",
                            collection.groupBy(x => x.Brand).select(x => ({ category: x.key, value: x.items.count() })).toArray(),
                            "{0:n0}",
                            `#: category #: \n #: value #`,
                            undefined);
                }
