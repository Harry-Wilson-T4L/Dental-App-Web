namespace DentalDrill.CRM.Controls.Reporting {
    ...
    export class PieChart extends ChartBase {
        private _optionsOverride: (options: kendo.dataviz.ui.ChartOptions) => void;

        constructor(container: ChartsCollectionContainer) {
            super(container);
        }

        override(method: (options: kendo.dataviz.ui.ChartOptions) => void): PieChart {
            this._optionsOverride = method;
            return this;
        }

        initialize(title: string, data: any[], format: string, template: string, maxValue: number): void {
            const options: kendo.dataviz.ui.ChartOptions = {
                title: {
                    position: "bottom",
                    text: title,
